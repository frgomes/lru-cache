...
object LRUCacheTest extends DefaultRunnableSpec {
  import zio._
  ...
  def spec = suite("LRUCache")(
    testM("can't be created with non-positive capacity") {
      assertM(LRUCache.make[Int, Int](0).run)(
  ...
  private def get[K, V](lruCache: LRUCache[K, V], key: K): ZIO[Console, IOException, Unit] =
    (for {
  ...
  private def put[K, V](lruCache: LRUCache[K, V], key: K, value: V): ZIO[Console, IOException, Unit] =
